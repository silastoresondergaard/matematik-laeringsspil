...
func _process(_delta: float):
	if player != null:
		# Tjek om spilleren har active variablen
		if "active" in player:
			if player.active:
				# Spiller er aktiv - frys grafen
				if not frozen:
					frozen = true
					frozen_position = global_position
				global_position = frozen_position
			else:
				# Spiller er ikke aktiv - følg spilleren
				frozen = false
				global_position = player.global_position + offset_from_player
		else:
			# Fallback til gammel logik
			if frozen:
				global_position = frozen_position
			else:
				global_position = player.global_position + offset_from_player
...
func _draw():
	...
	var status = status_text + " | Range: [-1,0] | Funktion: " + current_function
	draw_string(ThemeDB.fallback_font, graph_pos + Vector2(10, graph_height + 20), status, HORIZONTAL_ALIGNMENT_LEFT, -1, 14, Color.ORANGE)
